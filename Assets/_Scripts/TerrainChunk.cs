...
public class TerrainChunk {
    ...
    // CONSTRUCTOR
    public TerrainChunk(Vector2 botLeftPoint, float chunkSize, int levelOfDetailIndex, NoiseSettings noiseSettings) {
        /* PARAM DESCRIPTIONS
        ...
        // Creating the Mesh
        Mesh newMesh = GenerateMesh(botLeftPoint, chunkSize, levelOfDetailIndex, noiseSettings);
    ...
    public void UpdateNoise() {
        Mesh newMesh = GenerateMesh(botLeftPoint, chunkSize, levelOfDetailIndex, noiseSettings);
        PerlinNoise.ApplyPerlinNoise(botLeftPoint * CHUNK_SIZE, ref newMesh, noiseSettings);
        chunkObject.GetComponent<MeshFilter>().mesh = newMesh;
    }

    public void UpdateTerrainChunkLOD(int LODIndex) {
        // EXXY
        Mesh newMesh = GenerateMesh(botLeftPoint, chunkSize, LODIndex, noiseSettings);
